...
class DeribitClient():
    ...
    def request(self, action, data):
        ...
        if action.startswith("/api/v1/private/"):
            ...
            response = self.session.get(
                self.url + action, params=data, verify=True)

        if response.status_code != 200:
            raise Exception(
                "Wrong response code: {0}".format(response.status_code))
    ...
    def generate_signature(self, action, data):
        ...
        sorted_signature_data = OrderedDict(
            sorted(signature_data.items(), key=lambda t: t[0]))
